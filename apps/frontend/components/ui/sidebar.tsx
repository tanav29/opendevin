...
function SidebarMenuSkeleton({
  ...
  showIcon?: boolean
  /**
   * Width of the text line. Upstream randomises this, which makes the server
   * and client render different HTML and trips hydration on every load — so
   * callers vary it instead.
   */
  width?: string
}) {
  return (
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "mx-0 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border ml-2 pl-2 py-0.5 group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
    />
